...
rsge_error_e rsge_game_init(rsge_elglr_t* elglr) {
	rsge_error_e err = rsge_model_fromFile(&objects,"rsge@models/simple.xml");
	if(err != RSGE_ERROR_NONE) return err;
	list_t* model_vertData;
	err = rsge_elglr_mkvertarray(elglr,objects,&model_vertData);
	if(err != RSGE_ERROR_NONE) return err;
	err = rsge_elglr_buffertexs(elglr);
	if(err != RSGE_ERROR_NONE) return err;
