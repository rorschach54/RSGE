...
void main() {
	...
	applyDiffuse();
	applySpecular();
	fragmentColor = min(max(fragmentColor,0),1);
	fragmentColor *= lightAmount();
}

void applyDiffuse() {
	float dotProduct = max(dot(eyeSpaceNormal.xyz,-eyeSpaceLightVector.xyz),0);
	fragmentColor = max(vec4(diffuse*dotProduct*pointLight.intensity),0)*attenuation;
}
...
void applySpecular() {
	...
	fragmentColor += vec4(specular.xyz,1)*pow(dotProduct*pointLight.intensity,vec4(specular.www,1))*attenuation;
}
vec4 depthToEyeSpace(mat4 projection,float depth) {
...
void main() {
	...
	eyeSpaceLightVector = vec4(normalize(eyeSpacePosition.xyz-eyeSpaceLightPosition.xyz),0);
	lightDistance = distance(eyeSpaceLightPosition,eyeSpacePosition);
	attenuation = min(1.0/pow(lightDistance,1.05),1.0);
	
	applyDiffuse();
	applySpecular();
	fragmentColor = min(max(fragmentColor,0.0),1.0);
	fragmentColor *= lightAmount();
}

void applyDiffuse() {
	float dotProduct = max(dot(eyeSpaceNormal.xyz,-eyeSpaceLightVector.xyz),0.0);
	fragmentColor = max(vec4(diffuse*dotProduct*pointLight.intensity),0.0)*attenuation;
}

void applySpecular() {
	// Blinn-Phong
	vec3 halfwayVector = normalize(-eyeSpaceLightVector.xyz-normalize(eyeSpacePosition.xyz));
	float dotProduct = max(dot(halfwayVector,eyeSpaceNormal.xyz),0.0);
	fragmentColor += vec4(specular.xyz,1)*pow(dotProduct*pointLight.intensity,vec4(specular.www,1))*attenuation;
}
vec4 depthToEyeSpace(mat4 projection,float depth) {
	return vec4(normalize(eyeSpaceRay.xyz)*depth*camera.zFar,1);
...
float lightAmount() {
	...
	//This is a sphere
	vec3 sampleList[13];
	sampleList[0] = vec3(0,0,0);
	sampleList[1] = vec3(0.185011744,0.000050000,0.982736290);
	sampleList[2] = vec3(0.543722034,0.354457319,0.760740697);
	sampleList[3] = vec3(-0.0940286964,0.341989040,0.934987724);
	sampleList[4] = vec3(-0.666421711,0.478428960,0.571828604);
	sampleList[5] = vec3(-0.251662046,0.915312707,-0.314434320);
	sampleList[6] = vec3(0.830358803,0.541318417,0.132206663);
	sampleList[7] = vec3(0.188106239,0.679270804,0.709370971);
	sampleList[8] = vec3(-0.304940492,0.736509264,0.603792489);
	sampleList[9] = vec3(-0.381129295,0.920524716,0.0858761966);
	sampleList[10] = vec3(0.264770985,0.956115067,-0.125460610);
	sampleList[11] = vec3(0.539285064,0.766062260,0.349743187);
	sampleList[12] = vec3(0.0589527227,0.947874784,0.313142180);
