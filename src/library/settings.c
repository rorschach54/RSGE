...
rsge_error_e rsge_settings_getint(char* key,int* val) {
	rsge_error_e err = rsge_settings_load();
	if(err != RSGE_ERROR_NONE) return err;
	config_setting_t* root = config_root_setting(&rsge_libconfig_cfg);
	config_setting_t* cfg = config_lookup(&rsge_libconfig_cfg,key);
	if(!cfg) return RSGE_ERROR_LIBCONFIG;
	*val = config_setting_get_int(cfg);
...
rsge_error_e rsge_settings_setint(char* key,int val) {
	...
	config_setting_t* cfg = config_lookup(&rsge_libconfig_cfg,key);
	if(!cfg) return RSGE_ERROR_LIBCONFIG;
	config_setting_set_int(cfg,val);
